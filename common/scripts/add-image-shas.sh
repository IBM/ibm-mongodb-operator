...
EXPORTER_SHA=$(docker pull $EXPORTER_MONGODB_IMAGE | grep Digest | cut -d ':' -f3)
...
gsed -i "s/ibm-mongodb-exporter@sha.*/ibm-mongodb-exporter@sha256:$EXPORTER_SHA/g" deploy/olm-catalog/ibm-mongodb-operator/$CSV_VERSION/ibm-mongodb-operator.v$CSV_VERSION.clusterserviceversion.yaml
